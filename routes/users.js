        ...
        bcrypt.genSalt(10, (err, salt) =>
          bcrypt.hash(newUser.password, salt, (err, hash) => {
            ...
            newUser
              .save()
              .then(user => {
                req.flash('success_msg', 'Käyttäjä rekisteröity'); // Poista Reactin jälkeen
...
router.post('/login', (req, res, next) => {
  passport.authenticate('local', {
    ...
    failureFlash: true // Poista kun React
  })(req, res, next);
});
...
router.get('/logout', (req, res) => {
  req.logout();
  req.flash('success_msg', 'Kirjauduit ulos'); // Poista kun React
  res.redirect('/users/login');
});
