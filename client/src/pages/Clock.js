...
const Front = () => {
  ...
  //   try {
  //     const fetchedUsers = await axios.get('http://localhost:5555/users');
  //     setAllUsers(fetchedUsers.data);
  ...
  const handleAddUser = async user => {
    try {
      ...
      const userToAdd = await axios.post(
  ...
  return (
    <div>
      ...
      <Register addUser={handleAddUser} />
    </div>
  );
};
